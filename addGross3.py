...
X = pd.read_csv('uniqueMetadata.csv')
...
#        cSimpleR = fuzz.ratio(femaleName,maleName)
        cTokenR = fuzz.token_set_ratio(femaleName,maleName)
        if (tokenR < cTokenR):
            tokenR = cTokenR
            tokenI = i
...
#simple = open('simple.csv')
#token = open('simple.csv')
#simpleLookTable.to_csv('simple.csv')
setLookTable = tokenLookTable
setLookTable.to_csv('set.csv')
